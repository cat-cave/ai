...
function setExec(
  ...
  Object.defineProperty(handle, 'exec', { configurable: true, value: exec })
}

function createHandle(): DockerHandle {
  const docker = new Docker()
  let handle: DockerHandle
  handle = new DockerHandle({
    docker,
    container: docker.getContainer('test'),
    workdir: '/workspace',
    forkFactory: () => Promise.resolve(handle),
    removeOnDestroy: false,
  })
  return handle
}

describe('DockerHandle.fs.lstat', () => {
  it.each([
    '/workspace/missing',
  ...
  it('parses file, directory, symlink, and other metadata', async () => {
    ...
    const handle = createHandle()
    setExec(handle, async (command: string) => {
  ...
  it.each([' 81a4:12\n', '81a4:12\nextra\n'])(
    'rejects non-protocol whitespace/payload: %j',
    async (stdout) => {
      const handle = createHandle()
      setExec(handle, async () => ({ exitCode: 0, stdout, stderr: '' }))
      await expect(handle.fs.lstat!('/workspace/file')).rejects.toThrow(
        'invalid lstat output',
      )
    },
