...
/**
 * Wrapper for the snippets live test
 */
async function runSnippetsLiveWrapper(
  adapter: AnyTextAdapter | null,
  verbose: boolean,
): Promise<TestOutcome> {
  if (!adapter) {
  ...
  return {
    passed: result.passed,
    error: result.passed ? undefined : 'Snippets live test failed',
  }
}
...
export const TESTS: Array<TestDefinition> = [
  {
    ...
    name: 'Simulated',
    description:
      'Deterministic test with mock adapter (snippet creation + reuse)',
...
/**
 * Get all tests (no skip-by-default logic for snippets tests)
 */
