...
import { createFileSnippetStorage } from '../src/storage/file-storage'
import { createAlwaysTrustedStrategy } from '../src/trust-strategies'
import type { SnippetStorage } from '../src/types'

function makeSnippetInput(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    id: 'id-1',
    name: 'fetch_data',
    ...
    ...overrides,
  } as Parameters<SnippetStorage['save']>[0]
}

describe('createFileSnippetStorage', () => {
  let dir: string
  let storage: SnippetStorage

  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'snippets-test-'))
    storage = createFileSnippetStorage(dir)
  })
  ...
  it('creates the directory if it does not exist yet', async () => {
    ...
    const deepStorage = createFileSnippetStorage(nested)
    await expect(deepStorage.loadIndex()).resolves.toEqual([])
  })

  it('saves a snippet and round-trips it via get', async () => {
    const saved = await storage.save(makeSnippetInput({ name: 'alpha' }))
  ...
  it('persists snippets across independent storage instances pointing at the same dir', async () => {
    await storage.save(makeSnippetInput({ name: 'persistent' }))

    const second = createFileSnippetStorage(dir)
  ...
  it('separates code from metadata on disk', async () => {
    await storage.save(makeSnippetInput({ name: 'x', code: 'return 42;' }))
  ...
  it('preserves createdAt when updating an existing snippet', async () => {
    ...
    try {
      ...
      const first = await storage.save(makeSnippetInput({ name: 'x' }))
      vi.setSystemTime(new Date('2026-01-01T00:00:01.000Z'))
      const second = await storage.save(
        makeSnippetInput({ name: 'x', description: 'updated' }),
      )
  ...
  it('deletes a snippet including its directory and index entry', async () => {
    await storage.save(makeSnippetInput({ name: 'doomed' }))
    expect(await storage.delete('doomed')).toBe(true)
    expect(await storage.get('doomed')).toBeNull()
    expect(await storage.loadIndex()).toEqual([])
  })

  it('returns false when deleting a missing snippet', async () => {
    expect(await storage.delete('missing')).toBe(false)
  })

  it('searches via matching and respects limit', async () => {
    await storage.save(makeSnippetInput({ id: '1', name: 'github_stats' }))
    await storage.save(makeSnippetInput({ id: '2', name: 'npm_search' }))
    await storage.save(makeSnippetInput({ id: '3', name: 'other_github_tool' }))
  ...
  it('updateStats increments and applies the trust strategy', async () => {
    const alwaysTrusted = createFileSnippetStorage({
      directory: dir,
      trustStrategy: createAlwaysTrustedStrategy(),
    })
    await alwaysTrusted.save(
      makeSnippetInput({ name: 'x', trustLevel: 'untrusted' }),
    )
  ...
  it('updateStats is a no-op when snippet does not exist', async () => {
  ...
  it('exposes the configured trust strategy', () => {
    ...
    const s = createFileSnippetStorage({
      directory: dir,
