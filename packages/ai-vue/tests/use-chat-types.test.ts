...
describe('useChat() registered generic interrupt types', () => {
  it('keeps registered and external generic interrupts distinct', () => {
    ...
    const check = () => {
      ...
      type Interrupt = (typeof chat.interrupts.value)[number]
      ...
      // @ts-expect-error The public ref is readonly.
      chat.interrupts.value = []

      const existingTool = toolDefinition({
        name: 'vue-unregistered-tool',
      ...
      type WithoutRegistry = (typeof withoutRegistry.interrupts.value)[number]
      ...
      >().toEqualTypeOf<'vue-unregistered-tool'>()
      expectTypeOf<
